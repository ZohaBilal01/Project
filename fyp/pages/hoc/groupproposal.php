<?php 
   ...
   include('header.php');
    ...
    {
      mysqli_query($con,"UPDATE proposal SET marker=0");
    }
    ?>
    <!-- Content Wrapper -->
    ...
    <h4 class="btn btn-success btn-block btn-user btn-block">Groups Proposal Information</h4>
        ...
        $group=$row['group_id'];
			...
			<tr>
                <td><?php echo $group; 
                $sql=mysqli_num_rows(mysqli_query($con,"SELECT * FROM assign_proposal WHERE group_id='$group'"));
                if($sql == 0)
                {
                ?>
                <span class="label label-primary label-indicator animation-floating text-danger" style="font-size: 25px;"> * </span>
                ...
                <td><a href="proposaldetial.php?group_id=<?php echo $row['group_id']; ?>" class="btn btn-info">Proposal Information</a></td>
                <td><a href="assignproposal.php?group_id=<?php echo $row['group_id']; ?>" class="btn btn-success">Assign Proposal</a></td>
			</tr>
			<?php
			}
		?>
