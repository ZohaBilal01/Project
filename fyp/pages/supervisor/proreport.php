   ...
   include('header.php');
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <h4 class="btn btn-success btn-block btn-user btn-block">Proposal Reports </h4>
		...
		<?php
           ...
           $query=mysqli_query($con,"select * from proposal,groupp,register where groupp.group_id = '$group_id' and proposal.user_id=register.user_id");
			while($row=mysqli_fetch_array($query)){
				$num=mysqli_query($con,"select * from member where group_id='".$row['group_id']."'");
			?>
			<tr>
                <td><?php echo $row['group_id']; ?></td>
                <td><span class="badge"><?php echo mysqli_num_rows($num); ?></td>
		...
		<?php
            $group_id = $_GET['group_id'];
			$query=mysqli_query($con,"select * from evaluate_proposal,groupp where groupp.group_id = '$group_id'");
			while($row=mysqli_fetch_array($query))
            {
				$num=mysqli_query($con,"select * from member where group_id='".$row['group_id']."'");
			?>
